...
const Lookbook = () => {
  ...
  // 2. Sticky Horizontal Scroll Logic
  ...
  useEffect(() => {
    const calculateTravel = () => {
      ...
      // We want the last card to end at the right edge
      setTravelDistance(Math.max(stripWidth - viewportWidth, 0));
    };

    calculateTravel();
    window.addEventListener("resize", calculateTravel);
    
    // ResizeObserver catches width changes (like when images load)
    const observer = new ResizeObserver(calculateTravel);
  ...
  // Map 0-1 progress to the negative pixel travel distance
  const carouselX = useTransform(smoothHorizontalScroll, [0, 1], [0, -travelDistance]);

  return (
    <section ref={sectionRef} className="lookbook" id="lookbook">
      ...
      {/* ── Sticky section — Vertical scrolling drives Horizontal movement ── */}
...
//       <div ref={heroRef} className="lookbook-hero">
...
//             muted
//             playsInline
//             className="lookbook-hero-video"
//           />
//         </motion.div>
//         <div className="lookbook-hero-overlay" />

//         <div className="lookbook-title-position">
//           <motion.div
//             initial={{ opacity: 0, x: -80 }}
//             animate={isInView ? { opacity: 1, x: 0 } : {}}
//             transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }}
...
//           animate={isInView ? { scaleX: 1 } : {}}
//           transition={{ duration: 1.5, delay: 1, ease: [0.16, 1, 0.3, 1] }}
//           className="lookbook-divider"
//         />
//       </div>

//       <div ref={stickyContainerRef} className="lookbook-sticky-container relative">
...
//                 className="lookbook-carousel-item"
//               >
//                 <img
//                   src={img.src}
//                   alt={img.alt}
//                   className="lookbook-carousel-img"
//                 />
//               </motion.div>
//             ))}
//           </motion.div>
...
//   // Hero video parallax — this uses element-relative scroll so it's fine with Lenis
//   const { scrollYProgress: heroScroll } = useScroll({
//     target: heroRef,
//     offset: ["start end", "end start"],
//   });
...
//   /**
//    * CAROUSEL SCROLL SETUP
//    *
//    * We can't use Framer's useScroll() for the sticky container because
//    * Lenis intercepts native scroll events and Framer never sees them.
//    *
//    * Instead we:
//    * 1. Measure the sticky container's position in the document (offsetTop)
//    * 2. Measure how far the strip needs to travel (stripWidth - viewportWidth)
//    * 3. Use lenisScrollY (a MotionValue updated by Lenis every frame) to
//    *    compute the carousel's X position via useTransform
//    *
//    * lenisScrollY goes from 0 → maxScroll (full page height - viewport)
//    * We map the slice from containerStart → containerEnd to 0px → -travelDistance
//    */
//   const [containerStart, setContainerStart] = useState(1);
//   const [containerEnd,   setContainerEnd]   = useState(1);
//   const [travelDistance, setTravelDistance] = useState(0);

//   const measure = () => {
//     if (!stickyContainerRef.current || !stripRef.current) return;
 
//     // TRUE page offset — works regardless of CSS positioning on ancestors
...
//     // containerEnd = scroll position where the last frame of sticky is reached
//     const end = start + containerHeight - viewportHeight;
 
//     // How far the strip needs to travel horizontally
//     const stripWidth = stripRef.current.scrollWidth;
//     const travel     = Math.max(stripWidth - window.innerWidth, 0);


//     // ✅ FORCE HEIGHT MATCH
//   stickyContainerRef.current.style.height =
//     `${travel + window.innerHeight - 2}px`;
    
    
//     setContainerStart(start);
//     setContainerEnd(end);
...
//     // Re-measure after layout settles (fonts, images may shift things)
...
//   //     const stripWidth     = stripRef.current.scrollWidth;
//   //     const viewportWidth  = window.innerWidth;
//   //     const travel         = Math.max(stripWidth - viewportWidth, 0);

//   //     // Where in the page does the sticky container start/end?
//   //     const start = stickyContainerRef.current.offsetTop;
//   //     const end   = start + stickyContainerRef.current.offsetHeight - viewportWidth;
...
//   //   // Measure after fonts/images have loaded so offsetTop is accurate
//   //   measure();
//   //   window.addEventListener("resize", measure);

//   //   // Re-measure after a short delay in case layout shifts on load
...
//   /**
//    * Map lenisScrollY from [containerStart, containerEnd]
//    * to carouselX from [0, -travelDistance]
//    *
//    * clamp: true means the carousel won't overshoot at either end.
//    */
...
//   const [translateEnd, setTranslateEnd] = useState("0px");
  

// useEffect(() => {
//   const calculateEnd = () => {
//     if (!stripRef.current || !stickyContainerRef.current) return;

//     const stripWidth = stripRef.current.scrollWidth;
//     const viewportWidth = window.innerWidth;
//     const travelDistance = Math.max(stripWidth - viewportWidth, 0);

//     console.log({stripWidth,viewportWidth,travelDistance});
//     setTranslateEnd(`-${travelDistance}px`);

//     // 🔥 KEY FIX
//     stickyContainerRef.current.style.height =
//       `${travelDistance + window.innerHeight}px`;
//   };
...
// // const clampedProgress = useTransform(stickyScroll, (v) => {
...
// const carouselX = `calc(${progress} * ${translateEnd})`;


// // if (translateEnd === "0px") return null; // Don't render until we have the correct translation value to avoid a flash of unstyled content
//   return (
...
//             ref={stripRef}
//             className="lookbook__strip"
//             style={{  transform: `translateX(${carouselX})` }}
//           >
//             {images.map((img, i) => (
//               <motion.div
//                 key={i}
